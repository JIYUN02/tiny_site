...
def create_admin_account():
    with app.app_context():
        ...
        if not admin:
            ...
            # 환경 변수에서 비밀번호 가져오기 또는 랜덤 생성
            import os
            import secrets
            import string
            
            admin_password_plain = os.environ.get('ADMIN_INITIAL_PASSWORD')
            if not admin_password_plain:
                # 강력한 랜덤 비밀번호 생성 (16자리)
                chars = string.ascii_letters + string.digits + "!@#$%^&*()"
                admin_password_plain = ''.join(secrets.choice(chars) for _ in range(16))
            # 안전하게 파일에 저장 (로컬 개발 환경용)
            try:
                with open('.admin_password.txt', 'w') as f:
                    f.write(f"관리자 초기 비밀번호: {admin_password_plain}\n")
                    f.write("이 파일은 보안을 위해 즉시 삭제하세요.\n")
                os.chmod('.admin_password.txt', 0o600)  # 파일 권한 제한
                print("관리자 비밀번호가 .admin_password.txt 파일에 저장되었습니다. 보안을 위해 확인 후 파일을 삭제하세요.")
            except Exception as e:
                print("경고: 관리자 비밀번호를 파일에 저장할 수 없습니다.")
                print(f"관리자 비밀번호: {admin_password_plain} (보안을 위해 이 비밀번호를 안전한 곳에 저장하세요)")
...
app.config['MAX_CONTENT_LENGTH'] = 10 * 1024 * 1024  # 최대 10MB 파일
...
def validate_password(password):
    # 비밀번호 검증: 최소 8자, 소문자/숫자/특수문자 각각 하나 이상 포함
...
def handle_private_message(data):
    ...
    try:
        ...
        # 해당 사용자에게만 메시지 전달
        socketio.emit('new_private_message', message_data, room=receiver_id)
        socketio.emit('new_private_message', message_data, room=sender_id)
        
    except Exception as e:
        print(f"메시지 저장 중 오류: {str(e)}")

# 사용자가 접속했을 때 세션과 소켓 연결
@socketio.on('connect')
def handle_connect():
    if 'user_id' in session:
        user_id = session['user_id']
        join_room(user_id)  # 사용자 개인 룸에 조인
...
def handle_mark_as_read(data):
    ...
    try:
        ...
        # 읽음 상태 업데이트 알림
        socketio.emit('messages_read', {'sender_id': sender_id, 'receiver_id': receiver_id}, room=sender_id)
        
    except Exception as e:
        print(f"읽음 상태 업데이트 중 오류: {str(e)}")
...
def search_products():
    ...
    # 입력값 검증
    query = sanitize_input(query)  # XSS 방지
    
    # 정렬 옵션 검증 (화이트리스트 방식)
    allowed_sort_options = ['title', 'price_asc', 'price_desc']
    if sort_by not in allowed_sort_options:
        sort_by = 'title'  # 기본값으로 대체
...
def report():
    ...
    if request.method == 'POST':
        ...
        # 대상 유형에 따라 신고 카운트만 증가시킴 (자동 처리는 제거)
        if target_type == 'user':
...
def admin_dashboard():
    ...
    unprocessed_reports = cursor.fetchone()['count']
    ...
    return render_template('admin_dashboard.html', 
                          ...
                          unprocessed_reports=unprocessed_reports)  # 신고 알림 개수 추가
...
def admin_users():
    ...
    query = f"SELECT * FROM user ORDER BY {valid_columns[sort_by]} {valid_orders[order]}"
...
def admin_products():
    ...
    query = f"""
        ...
        FROM product p 
        JOIN user u ON p.seller_id = u.id 
        ORDER BY {valid_columns[sort_by]} {valid_orders[order]}
    """
    cursor.execute(query)
    products = cursor.fetchall()
...
def admin_report_action(report_id):
    ...
    if action == 'delete':
        ...
        # 신고 반려 - 해당 신고 삭제만 하고 대상은 변경하지 않음
        cursor.execute("DELETE FROM report WHERE id = ?", (report_id,))
        flash('신고가 반려되었습니다.')
...
def admin_search():
    query = request.args.get('query', '')
    search_type = request.args.get('type', 'all')  # all, user, product
    
    if not query:
        return render_template('admin_search.html', query='', results=None)
    db = get_db()
    cursor = db.cursor()
    ...
    return render_template('admin_search.html', query=query, results=results, type=search_type)


# 관리자 권한으로 모든 게시물 삭제 가능하도록 수정
